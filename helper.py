...
class Helper:
    ...
    def make_folders(self, continuation=False):
        log = create_logger()
        if self.params.log:
            try:
    ...
    def save_model(self, model=None, epoch=0, val_acc=0):
        print(self.params.folder_path)
        if self.params.save_model:
            epoch_path = f"{self.params.folder_path}/epoch_{epoch}"
            os.mkdir(epoch_path)
            logger.info(f"Saving model to {epoch_path}.")
            model_name = f'{epoch_path}/model_last.pt.tar'
